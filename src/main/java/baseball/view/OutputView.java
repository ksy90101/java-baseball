...
public class OutputView {
    ...
    public static void printResult(final CheckBallResponse checkBallDto) {
        ...
        if (checkBallDto.isFinished()) {
            if (checkBallDto.isSuccess()) System.out.println("3개의 숫자를 모두 맞히셨습니다.");
    ...
    public static void printGameRecords(final List<GameRecordsResponse> gameRecords) {
        ...
        for (final GameRecordsResponse gameRecord : gameRecords) {
            final String stringBuilder = "[" + gameRecord.id() + "] " +
                    "시작시간: " +
                    gameRecord.startAt().format(DateTimeFormatter.ofPattern(OutputView.DATE_TIME_FORMAT)) +
                    " / " +
                    "종료시간:" + gameRecord.endAt().format(DateTimeFormatter.ofPattern(OutputView.DATE_TIME_FORMAT)) +
                    " / " +
    ...
    public static void printGameRecord(final GameRecordResponse gameRecord) {
        ...
        gameRecord.playerRecordResponses().forEach(playerRecord -> {
            final String playerNumber = playerRecord.numbers().stream().map(String::valueOf).collect(Collectors.joining(""));
            ...
            if (playerRecord.isSuccess()) System.out.println("3개의 숫자를 모두 맞히셨습니다.");
        });

        System.out.println("-------기록 종료-------");
    }

    public static void printStatistics(final StatisticsResponse statistics) {
